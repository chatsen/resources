...
var users = <User>[
  ...
  User(
    ...
    badges: [
      ...
      UserBadge(
        badgeName: 'early_supporter',
      ), // , acquiredAt: DateTime.now(), updatedAt: DateTime.now()),
      UserBadge(
        badgeName: 'early_bird',
      ), // , acquiredAt: DateTime.now(), updatedAt: DateTime.now()),
    ],
  ),
  User(
...
void main(List<String> arguments) async {
  for (var user in users.where((user) => user.id == '0')) {
    ...
    print(
      'User(name: \'${user.name.toLowerCase()}\', id: \'${user.id}\', badges: []),',
    );
  }

  await File('assets/data.json').writeAsString(
    JsonEncoder.withIndent('  ').convert(Data(users: users, badges: badges)),
  );
}
